...
from qinghai_rag.source_discovery import (
    ...
    discover_ihchina_inheritor_catalog,
)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Discover Qinghai inheritor pages from the official national ICH catalog"
    )
    ...
    parser.add_argument(
        "--output",
        default=str(PATHS.interim / "source_candidates_ihchina_inheritors.jsonl"),
    )
    parser.add_argument(
        "--offline-json",
    ...
    if args.offline_json:
        ...
        discovered = candidates_from_ihchina_inheritor_payload(payload)
    else:
        discovered = discover_ihchina_inheritor_catalog(
