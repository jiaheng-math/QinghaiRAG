...
from qinghai_rag.guide_discovery import GUIDE_SECTIONS, discover_guide_tourism
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Discover article metadata from the official Guide County tourism portal"
    )
    parser.add_argument("--sections", nargs="+", choices=sorted(GUIDE_SECTIONS))
    parser.add_argument("--timeout", type=float, default=60.0)
    parser.add_argument("--interval", type=float, default=1.0)
    parser.add_argument("--max-pages", type=int, default=None)
    parser.add_argument("--contact", default=None)
    parser.add_argument("--transport", choices=["curl", "requests"], default="curl")
    parser.add_argument(
        "--no-env-proxy",
        action="store_true",
        help="Ignore proxy environment variables when accessing the official county site",
    )
    parser.add_argument(
        "--output",
        default=str(PATHS.interim / "source_candidates_guide_tourism.jsonl"),
    )
    parser.add_argument("--register", action="store_true")
    args = parser.parse_args()

    discovered, metadata = discover_guide_tourism(
