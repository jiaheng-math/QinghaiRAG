...
def _located_fact(fact_id: str, subject: str, region: str) -> FactRecord:
    payload = _declared_fact(fact_id, subject, region).model_dump(mode="json")
    payload.update({"predicate": "located_in", "object_type": "REGION"})
...
def test_regional_aggregation_names_the_circulation_area_relation():
    questions = generate_regional(
        [_located_fact("located", "古法沙棘火锅汤炮制技艺", "青海省")],
        target=10,
    )

    assert len(questions) == 1
    assert "流传" in questions[0].question
